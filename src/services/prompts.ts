...
export const MAINTENANCE_ACTION_API: readonly MaintenanceActionDefinition[] = [
  ...
  maintenanceAction('unit_create', ['session_id', 'message_ids', 'reason'], { session_id: 'string', message_ids: 'string[]', title: 'string?', summary: 'string?', concept_ids: 'string[]' }, '从同一 Session 的一组未归档消息创建阅读片段；可同时设置标题、摘要和主题归属'),
...
- Message 可以不归属任何 Concept；只标记有直接证据的消息，不要为了覆盖全部消息、凑满数量或重复同一主题而逐条复制 membership。每个新候选至少要被一条 Message membership 引用。只有输入是完整 Session，且主题确实概括整个会话时，才添加 Session membership。
...
- hierarchy 中 source 是直接父主题、target 是直接子主题。只有 target 的语义范围严格包含于 source，且二者是稳定的“上位概念/下位概念”或类别→成员关系时才能使用；因果、先后、组成步骤、支持/依赖、实现/使用、比较、同会话出现或一般相关都不是 hierarchy。方向不确定时省略关系，不要同时返回可由其他边推导出的传递关系。
...
export function buildConversationPrompt(input: {
  ...
  availableUnits?: Array<{ id: string; title: string; summary: string }>
  conceptLimit?: number
}): string {
  const disclosureText = formatDisclosureContext(input.disclosure)
  const conceptLimit = promptConceptLimit(input.conceptLimit)
  const unitExample = input.availableUnits?.length
    ? `{"unit_id":"${input.availableUnits[0].id}","concept_ids":["已有 Concept refID"],"concepts":[]}`
    : '{"title":"新阅读片段标题（必填）","summary":"新阅读片段摘要（必填）","concept_ids":["已有 Concept refID"],"concepts":[{"name":"仅属于该阅读片段的新 Concept 名称","summary":"不超过 120 个中文字符的主题摘要","aliases":[]}]}'
...
当前 Session 可复用的阅读片段：
${input.availableUnits?.length ? input.availableUnits.map((unit) => `- ${unit.id} · ${unit.title || '未命名阅读片段'}：${unit.summary || '暂无摘要'}`).join('\n') : '（当前 Session 还没有阅读片段；本轮回答必须形成一个新的阅读片段）'}
...
请只返回 JSON，格式如下：
{"answer":"完整回答，包含 [[nexus:suggested:推荐词]]推荐词[[/nexus]]和[[nexus:existing:已存在主题]]已存在主题[[/nexus]]（可包含 Markdown）","session_title":"不超过 60 个字符的 Session 标题","session_summary":"不超过 120 个字符的 Session 滚动摘要","concepts":[{"client_ref":"new:1","name":"新 Concept 名称","summary":"不超过 120 个中文字符的主题摘要","aliases":[]}],"memberships":[{"target_type":"session|message","target_id":"上面给出的 Session 或 Message ID","concept_ids":["已有 Concept refID 或 new:1"]}],"relations":[{"source":"直接父 Concept refID 或 new:1","target":"直接子 Concept refID 或 new:1","type":"hierarchy","status":"proposed"}],"units":[${unitExample}],"disclosure_requests":[]}

session_title 和 session_summary 概括当前完整 Session，而不只是本轮问题；已有标题合适时原样返回。旧任务可以省略这两个字段，应用会保留已有值。units 打包本轮用户问题和回答，不表示知识主题层级，并且本轮绝不能返回空数组：第一轮必须新建，后续轮必须复用当前 Session 已有的 unit_id 或在证据边界变化时新建。不要返回解释文字。`)
}
