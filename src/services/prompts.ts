...
主题标记约定：回答正文中提到输入目录里已有的知识主题时，可用 [[nexus:existing:主题名称]]原文[[/nexus]] 标记；你认为值得用户继续探索、但尚未确认存在的主题，用 [[nexus:suggested:主题名称]]原文[[/nexus]] 标记。标记中的原文必须与显示文本一致，不能嵌套或跨段；不确定时不要添加标记。应用会把已有主题显示为蓝色下划线、建议主题显示为黄色下划线。
