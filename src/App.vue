      ...
      <section class="content-shell">
        ...
        <section v-else-if="activeView === 'graph'" class="view-panel graph-view">
          ...
          <div class="graph-layout">
            ...
            <aside class="graph-controls surface-section"><div class="panel-heading"><div><span class="eyebrow">VIEW</span><h3>显示选项</h3></div><SlidersHorizontal :size="17" /></div><label class="toggle-row"><span><strong>知识单元</strong><small>点击知识主题时始终展开</small></span><input v-model="graphShowUnits" type="checkbox" /></label><label class="toggle-row"><span><strong>消息与会话链</strong><small>显示同一会话中的原始消息及顺序连接</small></span><input v-model="graphShowMessages" type="checkbox" /></label><label class="toggle-row"><span><strong>探讨与流程会话</strong><small>显示分类后保留的非主题会话</small></span><input v-model="graphShowRetainedSessions" type="checkbox" /></label><label class="toggle-row"><span><strong>待确认关系</strong><small>以虚线呈现建议关系</small></span><input v-model="graphShowProposed" type="checkbox" /></label><div class="graph-mini-stats"><div><strong>{{ graphOverview.concepts }}</strong><span>知识主题</span></div><div><strong>{{ graphOverview.units }}</strong><span>知识单元</span></div><div><strong>{{ graphOverview.edges }}</strong><span>关系</span></div></div><div class="graph-control-note"><GitBranch :size="15" /><span>父子关系会增加吸引力，子知识主题倾向靠近父节点；拖拽只改变视图位置。</span></div></aside>
          </div>
        </section>
