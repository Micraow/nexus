...
<div class="merge-form"><SearchSelect v-model="mergeTargetId" :options="[{ value: '', label: '选择目标知识主题' }, ...store.activeConcepts.filter((item) => item.id !== selectedConcept?.id).map((concept) => ({ value: concept.id, label: concept.name, hint: concept.summary }))]" aria-label="合并目标" /><button class="button danger-button" :disabled="!mergeTargetId" @click="mergeSelectedConcept"><GitBranch :size="14" />合并</button></div>
        ...
        <section v-if="activeView === 'tasks' && selectedTask?.type === 'maintenance' && maintenanceOverallReason" class="surface-section maintenance-task-results" aria-labelledby="maintenance-task-reason-title"><div class="section-heading"><div><span class="eyebrow">MAINTENANCE RESULT</span><h3 id="maintenance-task-reason-title">模型判断</h3></div><span v-if="!maintenanceSuggestions.length" class="status-label label-success">无建议变更</span><span v-else class="status-label label-warning">{{ maintenanceSuggestions.length }} 条建议</span></div><p class="maintenance-overall-reason">{{ maintenanceOverallReason }}</p></section>
    ...
    <section v-if="maintenancePanelOpen && (((activeView === 'concepts' || activeView === 'graph') && selectedConcept) || (activeView === 'sessions' && selectedSession) || (activeView === 'tasks' && (maintenanceSuggestions.length || selectedTask?.type === 'maintenance')) || (activeView === 'graph' && store.selectedUnits.length))" class="maintenance-panel surface-section" aria-label="知识维护">
      ...
      <div v-if="activeView === 'sessions' && selectedSession" class="maintenance-scope">
        <div class="maintenance-scope-copy"><strong>全图扫描 · 优先关注「{{ displayText(selectedSession.title, '未命名会话') }}」</strong><span>当前会话只是优先证据范围，不会隐藏其他主题</span></div>
