...
const pendingConversationBranch = ref<(NavTreeNode & { started: boolean }) | null>(null)
...
const fullscreenTitle = computed(() => {
  ...
  if (target?.kind === 'concept') return store.concepts.find((concept) => concept.id === target.conceptId)?.name ?? '主题消息'
...
const maintenanceOverallReason = computed(() => {
  if (!selectedTask.value || selectedTask.value.type !== 'maintenance' || !selectedTask.value.parsedResult) return ''
  try {
    const parsed = JSON.parse(selectedTask.value.parsedResult) as { reason?: unknown }
  ...
  if (composerFollowUp.value) {
    try {
      if (pendingConversationBranch.value && pendingConversationBranch.value.parentId === composerFollowUp.value.nodeId) {
        pendingConversationBranch.value.started = true
      }
        ...
        <div class="topbar-actions">
          <div class="global-search" :class="{ expanded: showSearch }">
            ...
            <div v-if="showSearch && searchQuery" class="search-popover">
              ...
              <div v-if="searchResults.units.length" class="search-group"><span class="search-group-title">阅读片段</span><button v-for="unit in searchResults.units.slice(0, 4)" :key="unit.id" @click="openUnit(unit.id); showSearch = false"><BookOpen :size="14" />{{ unit.title || '未命名阅读片段' }}</button></div>
      ...
      <section class="content-shell">
        ...
        <section v-if="activeView === 'overview'" class="view-panel overview-view">
          ...
          <div v-if="!activeConversationSession" class="overview-support-grid">
            <section class="surface-section support-card"><div class="section-heading"><div><span class="eyebrow">RECENT SESSIONS</span><h3>最近会话</h3></div><button class="text-button" @click="setView('sessions')">查看全部 <ArrowRight :size="14" /></button></div><div class="recent-list"><button v-for="session in store.activeSessions.slice(0, 4)" :key="session.id" class="recent-row" @click="session.source === 'in_app' ? openConversationSession(session.id) : selectSession(session.id)"><div class="session-avatar"><History :size="15" /></div><div class="row-main"><strong>{{ session.title }}</strong><span>{{ session.platform }} · {{ session.messageCount }} 条消息 · {{ session.unitCount }} 个阅读片段</span></div><ChevronRight :size="16" /></button><div v-if="!store.activeSessions.length" class="empty-inline">还没有会话，先提出你的第一个问题。</div></div></section>
        ...
        <section v-else-if="activeView === 'concepts'" class="view-panel concepts-view">
          <div class="page-toolbar">
            ...
            <button class="button secondary-button" @click="setView('graph')"><Network :size="15" />在图谱中查看</button>
          </div>
            ...
            <div ref="conceptPageDetail" class="concept-detail surface-section" tabindex="-1">
              <template v-if="selectedConcept">
                <div class="detail-header">
                  <div><span class="eyebrow">KNOWLEDGE TOPIC</span><h3>{{ selectedConcept.name }}</h3></div>
                ...
                <div class="concept-page-relation-grid">
                  <section class="detail-subsection">
                    ...
                    <div v-if="conceptParentQuery.trim() && conceptParentCandidates.length" class="relation-candidates" role="listbox" aria-label="父主题候选">
                      <button v-for="candidate in conceptParentCandidates" :key="candidate.id" class="relation-candidate" @click="addConceptParent(candidate.id)"><Plus :size="13" /><span>{{ candidate.name }}</span><small>{{ linkedUnitCount(candidate.id) }} 个阅读片段</small></button>
                    </div>
                  ...
                  <section class="detail-subsection">
                    ...
                    <div v-if="conceptChildQuery.trim() && conceptChildCandidates.length" class="relation-candidates" role="listbox" aria-label="子主题候选">
                      <button v-for="candidate in conceptChildCandidates" :key="candidate.id" class="relation-candidate" @click="addConceptChild(candidate.id)"><Plus :size="13" /><span>{{ candidate.name }}</span><small>{{ linkedUnitCount(candidate.id) }} 个阅读片段</small></button>
                    </div>
                ...
                <section class="detail-subsection">
                  ...
                  <button v-for="session in selectedConceptSessions" :key="session.id" class="mini-unit-row" @click="openFullscreenSession(session.id)"><History :size="14" /><span>{{ session.title }}</span><small>{{ session.messageCount }} 条消息</small><Maximize2 :size="14" /></button>
                ...
                <section class="detail-subsection">
                  ...
                  <button v-for="unit in selectedConceptUnits" :key="unit.id" class="mini-unit-row" @click="openUnit(unit.id)"><BookOpen :size="14" /><span>{{ unit.title || '未命名阅读片段' }}</span><small>{{ sessionForUnit(unit)?.title }}</small><ChevronRight :size="14" /></button>
                ...
                <section class="detail-subsection">
                  ...
                  <div class="concept-message-preview">
                    <div v-for="message in selectedConceptMessages.slice(0, 12)" :key="message.id" class="mini-unit-row">
                      ...
                      <small>{{ store.sessions.find((session) => session.id === message.sessionId)?.title || '未知会话' }}</small>
                    </div>
                  </div>
...
<div class="merge-form"><SearchSelect v-model="mergeTargetId" :options="[{ value: '', label: '选择目标知识主题' }, ...store.activeConcepts.filter((item) => item.id !== selectedConcept?.id).map((concept) => ({ value: concept.id, label: concept.name, hint: concept.summary }))]" aria-label="合并目标" /><button class="button danger-button" :disabled="!mergeTargetId" @click="mergeSelectedConcept"><GitBranch :size="14" />合并</button></div>
    ...
    <button v-if="!maintenancePanelOpen && (((activeView === 'concepts' || activeView === 'graph') && selectedConcept) || (activeView === 'sessions' && selectedSession) || (activeView === 'tasks' && (maintenanceSuggestions.length || selectedTask?.type === 'maintenance')) || (activeView === 'graph' && store.selectedUnits.length))" class="maintenance-launcher button secondary-button" aria-label="打开知识维护" title="打开知识维护" @click="openMaintenancePanel"><Sparkles :size="15" />知识维护</button>

    <section v-if="maintenancePanelOpen && (((activeView === 'concepts' || activeView === 'graph') && selectedConcept) || (activeView === 'sessions' && selectedSession) || (activeView === 'tasks' && (maintenanceSuggestions.length || selectedTask?.type === 'maintenance')) || (activeView === 'graph' && store.selectedUnits.length))" class="maintenance-panel surface-section" aria-label="知识维护">
      ...
      <div v-if="store.selectedUnits.length" class="maintenance-scope">
        <div class="maintenance-scope-copy"><strong>已选 {{ store.selectedUnits.length }} 个阅读片段</strong><span>按当前上下文顺序检查关联和标题摘要</span></div>
    ...
    <aside v-if="isDetailOpen && (selectedConcept || selectedUnit || selectedMessage)" ref="detailDrawer" class="detail-drawer" :class="{ open: isDetailOpen }">
      ...
      <div v-if="selectedUnit" class="drawer-content"><div class="drawer-tags"><span class="soft-tag">阅读片段</span><span class="soft-tag">{{ sessionForUnit(selectedUnit)?.title }}</span></div><label class="field-label" for="unit-title">标题 <small>≤30 字</small></label><input id="unit-title" v-model="unitDraftTitle" class="drawer-input" maxlength="30" /><label class="field-label" for="unit-summary">摘要 <small>≤120 字</small></label><textarea id="unit-summary" v-model="unitDraftSummary" class="drawer-textarea" maxlength="120" /><button class="button primary-button full-button" @click="saveUnit"><Check :size="15" />保存片段</button><div class="drawer-section"><div class="subsection-title"><strong>关联知识主题</strong><span>{{ store.unitConceptNames(selectedUnit.id).length }}</span></div><div class="chip-list"><button v-for="conceptId in store.unitConcepts.filter((link) => link.unitId === selectedUnit?.id).map((link) => link.conceptId)" :key="conceptId" class="concept-chip" @click="openConcept(conceptId)">{{ store.concepts.find((concept) => concept.id === conceptId)?.name }}<X :size="12" @click.stop="removeConceptFromUnit(selectedUnit!.id, conceptId)" /></button><span v-if="!store.unitConceptNames(selectedUnit.id).length" class="muted">暂无关联</span></div><div class="add-inline"><input v-model="newUnitConcept" placeholder="添加知识主题" @keyup.enter="addConceptToSelectedUnit" /><button class="icon-button" aria-label="添加知识主题" title="添加知识主题" @click="addConceptToSelectedUnit"><Plus :size="15" /></button></div></div><div class="drawer-section"><div class="subsection-title"><strong>包含消息</strong><span>{{ store.unitMessages(selectedUnit.id).length }}</span></div><div class="message-stack"><article v-for="message in store.unitMessages(selectedUnit.id)" :key="message.id" class="message-card" :class="message.role"><span>{{ message.role === 'user' ? '你' : message.role === 'assistant' ? 'AI' : '系统' }}</span><div class="md-body" v-html="renderedMessageContent(message.content, message)" @click="handleRenderedClick" @keydown.enter.prevent="handleRenderedClick" /></article></div></div><button class="text-button" @click="selectedUnitId = null; setView('sessions')"><ArrowRight :size="14" />打开所属会话</button></div>
