...
- 图谱布局持久化：节点拖拽结束把最后坐标写入 `graph_layout` 作为后续布局种子，松手后继续参与力向；视口平移/缩放防抖后写入单行 `graph_viewport` 表，刷新后恢复坐标。“重置布局”清除这两类记录并重新计算。拖拽指针先按当前 zoom 变换反算到图谱坐标，按住期间只同步节点 `x/y` 并立即重绘，不设置永久 `fx/fy`，让弹簧、排斥和碰撞力持续生效，同时阻止画布平移竞争；释放后让关联节点短暂回弹。快照变化触发的重渲染以 d3 的实时变换恢复视口，持久化视口只在组件挂载时应用；store 保存/重置视口时会同步内存值，避免过期的 prop 把缩放拉回旧状态，“重置布局”通过递增组件 key 整体重建实现。
