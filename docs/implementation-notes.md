...
- 知识维护任务始终扫描整个 active Concept 图谱；从主题、会话或阅读片段入口触发时，这些对象只作为 Prompt 中的附加关注范围。Prompt 提供全部一级主题及直接子主题引用，并明确根节点是例外、新主题优先匹配最窄父主题、related 不能代替 hierarchy。维护结果支持创建/编辑/移动/解除 hierarchy/归档 Concept，应用前做 DAG 环检测并通过快照事务记录，可撤销。
...
- 对话结果允许返回最多两条有直接证据的 hierarchy/related 关系；应用以 `proposed` 写入并复用统一确认、拒绝与环检测流程，不再静默丢弃关系。
...
- 会话工作区按当前导航节点的祖先路径投影消息：切换主题会切换到另一条分支，不把新问题线性追加到旧分支尾部。路径上的问答以稳定的层叠卡片呈现，当前节点卡片位于最前并高亮；卡片偏移只使用 `transform`，在 `prefers-reduced-motion: reduce` 下关闭位移/旋转。Session 仍保留完整 Message 顺序供检索和导出，分支视图只是显示投影。
