...
- 知识主题目录左栏工具栏提供“新建知识主题”入口：打开表单时若左侧树已有选中主题，默认把该主题设为新主题的父级并自动展开其分支；也可在搜索选择框中改选其他现有父主题或“根主题（无父级）”。名称、摘要和说明写入本地 Concept，创建后自动选中新主题并打开详情；同名 active 主题不会重复创建。
...
- 知识维护面板的“执行自定义维护指令”是独立作用域：它只为完成用户输入的目标披露必要证据，不把未关联分支、阅读片段覆盖或全图例行审计作为完成前置条件；“开始全图维护”仍保留完整图谱审计语义。定向任务沿用同一 MCP 动作白名单、ID 披露校验与可撤销应用流程。
...
- 维护响应可以按需返回 `disclosure_requests`；当模型明确选择 `coverage="partial"` 时，不相关的 pending 引用不会强制展开，但 `reason` 必须说明未覆盖范围。需要完整审计时，每轮最多处理 4 个引用、每个请求展开一层，剩余引用由本地状态机继续排队；维护任务最多 64 轮，仍未完成才进入人工检查。
...
- API 任务队列按配置并发数（1～16）批量执行，单任务最多进行三次传输请求（含超时、429 和 5xx 的指数退避）；对话任务的结构化校验失败会先保存修复 Prompt，再自动重新排队，最多修复两轮，不能把错误响应直接应用。其他任务仍需人工检查或重新排队。同一 Session 的直接 Concept 任务按输入 revision 串行，避免旧结果覆盖新归属；可选 KnowledgeUnit 元数据任务只在对应单元创建后串行。Prompt 粘贴任务保持人工逐项应用。并发数已在设置页提供手动数字输入（1～16），写回 `config.yaml`；同一 Session 的 revision 规则不受并发数影响。
- `llm.concept_limit` 由设置页手动配置为 `1～32`（默认 `8`），并同时传入 Concept、起源 Concept 和对话 Prompt；应用校验器按当前值拒绝超限响应，不静默截断。Concept 名称要求教材章节式的短、单一主题词组，最长 24 个 Unicode 字符；Prompt 输出前机械扫描“与”“和”“及”“、”“/”“／”六类分隔字符，默认要求拆分并用 hierarchy 表达共同上位主题。若确属不可拆分的固定名称，API/对话响应可在同一 Concept 对象一次性提供 `confidence`（0～1）和非空 `reason`，例如 `{"name":"喜羊羊与灰太狼","confidence":0.99,"reason":"这是作品正式固定名称，完整指向同一部动画。"}`；已披露主题前缀重合时，返回直接 hierarchy 或提供同样的证据字段即可，不再机械禁止。不能只删除连接词后继续合并。消息归属只保留有直接证据的稀疏 membership。
...
- 全图维护的动作白名单从任务 Prompt 的已展开 `content` 动态派生：只有带实体正文的 Concept、关系、别名、Session、Message、KnowledgeUnit 才能作为动作 ID；目录中仅有 title/summary 的导航引用仍不可写入。API 模式若缺少总体 `reason` 直接进入 `needs_review`，避免“空 suggestions”被误显示为已完成；Prompt 粘贴模式保留旧响应的兼容性提示，但新 Prompt 始终要求非空 reason。
...
- 图谱布局持久化：节点拖拽结束把最后坐标写入 `graph_layout` 作为后续布局种子，松手后继续参与力向；视口平移/缩放防抖后写入单行 `graph_viewport` 表，刷新后恢复坐标。“重置布局”清除这两类记录并重新计算。拖拽指针先按当前 zoom 变换反算到图谱坐标，按住期间只更新一个临时 drag-target 力的目标，不写入 `fx/fy` 或直接抢写节点位置，让弹簧、排斥和碰撞力持续生效且避免闪烁，同时阻止画布平移竞争；释放后让关联节点短暂回弹。快照变化触发的重渲染以 d3 的实时变换恢复视口，持久化视口只在组件挂载时应用；store 保存/重置视口时会同步内存值，避免过期的 prop 把缩放拉回旧状态，“重置布局”通过递增组件 key 整体重建实现。
...
- 对话链：本地草稿 → 创建 `conversation` 任务（API 为 `pending → running`，Prompt 粘贴保持 `pending` 等待人工回传）→ 校验结果；API 对话校验失败会自动使用保存的修复 Prompt 重试最多两轮，仍失败才进入 `needs_review`。成功后写入 assistant Message、导航树节点和本轮阅读片段归属。首轮没有片段时必须创建新片段且不得返回 `unit_id`；后续可逐字复用 Prompt 已列出的真实 `unit_id`，或省略 ID 创建新片段；每轮 `units` 都必须非空，卡片顶部显示当前片段。追问 Prompt 会展开本 Session 已访问的 Concept 祖先路径，供模型复用已有 refID。导入消息缺片段时由维护 `unit_create` 补建。API 流式对话会把 SSE 增量暂存于任务状态并实时显示在当前卡片，完成后再以完整 JSON 校验落库。非法结果进入 `needs_review`，重试或版本变化分别回到 `pending` 或 `stale`。没有摘要的已应用片段仍为 `ready`，不再伪装成等待分段。
