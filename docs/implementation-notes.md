...
- 图谱布局持久化：节点拖拽结束写入 `graph_layout`（固定坐标），视口平移/缩放防抖后写入单行 `graph_viewport` 表；刷新后恢复。“重置布局”清除这两类记录并重新计算。拖拽指针先按当前 zoom 变换反算到图谱坐标，按住期间同步节点 `x/y` 与 `fx/fy` 并立即重绘，同时阻止画布平移竞争；释放后保留固定位置并让关联节点短暂回弹。快照变化触发的重渲染以 d3 的实时变换恢复视口，持久化视口只在组件挂载时应用；store 保存/重置视口时会同步内存值，避免过期的 prop 把缩放拉回旧状态，“重置布局”通过递增组件 key 整体重建实现。
