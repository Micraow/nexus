...
- `llm.concept_limit` 由设置页手动配置为 `1～32`（默认 `8`），并同时传入 Concept、起源 Concept 和对话 Prompt；应用校验器按当前值拒绝超限响应，不静默截断。Concept 名称要求教材章节式的短、单一主题词组，最长 24 个 Unicode 字符；Prompt 输出前机械扫描“与”“和”“及”“、”“/”“／”六类分隔字符，默认要求拆分并用 hierarchy 表达共同上位主题；若确属不可拆分的固定技术名称，API/对话响应可在同一 Concept 对象一次性提供 `confidence`（0～1）和非空 `reason`，例如 `{"name":"PFC 与 ECN 协议栈","confidence":0.93,"reason":"该名称在本文中作为一个固定协议栈整体反复指代，拆分会丢失组合语义。"}`，不能只删除连接词后继续合并。消息归属只保留有直接证据的稀疏 membership。
