...
3. API 设置打开“流式传输对话”后，用 OpenAI 兼容 SSE 端点确认当前卡片在任务完成前出现“实时输出”和增量文本；任务结束后再校验完整 JSON、首轮阅读片段、单卡片/单根节点状态和任务成功落库。API Key 只保存在本机配置，不写进 fixture、截图或日志。
...
4. 用维护任务和对话任务分别核对 Prompt 中可见的 ID 目录。普通提取模型只能引用目录中的既有 Concept；对话任务还可引用同一 Session 其他探索分支已经建立归属的 active Concept（应用会把这些 ID 及祖先路径加入对话目录），以及同响应 `client_ref`、本轮 Session/Message/Unit 目标；不得用仅有 title/summary 的未披露节点或其他 Session ID。维护响应若 `suggestions=[]` 必须提供非空总体 `reason`，API 模式下所有维护响应均须提供 `reason`。为每条范围规则保留回归测试。
