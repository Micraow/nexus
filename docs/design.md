...
对话答案中的主题标记使用 `[[nexus:existing:主题名称]]回答中实际出现的词组[[/nexus]]` 和 `[[nexus:suggested:主题名称]]回答中实际出现的词组[[/nexus]]`。marker 正文必须逐字复制回答中真实出现的词组，不得使用“原文”等占位文字；已有主题以蓝色下划线呈现并可点击打开详情，建议探索主题以黄色下划线呈现；标记只影响展示，不改变事实归属。
