...
图谱采用渐进式披露：默认只投影没有任何未拒绝 hierarchy 父节点的 active 根 Concept；提议中的 hierarchy 父关系虽然默认不绘制，但仍定义结构上的父级，不能把子主题提升为根，打开待确认关系显示后才绘制该提议边。用户点击 Concept 主体后逐层显示其直接子节点。展开祖先路径后，路径上的节点保持可见；收起某个祖先会递归收起其后代，后代的事实关系仍保留在数据库中。`related` 显示不参与层级展开；Session/KnowledgeUnit/Message 开关控制附加节点，不能被 Concept 展开绕过。

知识主题目录左侧使用 hierarchy 树而不是平铺列表；主题行单击打开右侧内容，箭头按钮展开或收起当前分支。目录树不使用受视口高度限制的嵌套滚动区，长列表跟随页面主滚动，避免详情页较长时左侧可见区域被截断。会话探索树采用节点图样式：较大的圆点是导航主体，父子之间用细线连接，标签默认隐藏并在悬停或键盘聚焦时显示；圆点仍支持键盘选择，追问操作保留独立图标入口。过滤主题时保留匹配节点的祖先路径，避免结果脱离层级上下文。
...
维护动作目录同时暴露为 MCP `tools/list` 兼容的 `nexus_maintenance_*` 工具：每个工具只有 `name`、`description` 和严格的 `inputSchema`，其中 `additionalProperties=false`；`maintenanceMcpToolsList()` 提供标准 `{tools:[...]}` result envelope，API 模式还把同一目录作为 OpenAI-compatible function tools 发送给模型。工具调用按注册名称转换为本地 suggestion，再与 Prompt 粘贴模式共享白名单、ID、DAG 和快照事务校验；应用层仍保留带兼容别名和审核说明的完整目录。`create_concept` 可在一次原子调用中提交别名及一个或多个直接父主题；`set_hierarchy_parents` 可整体替换多父 DAG。所有工具调用都必须带非空 `reason`，未知工具、未知字段、重复别名、关系自环和层级成环在事务前拒绝。
...
用户可在知识主题目录左侧树中按当前选中层级手动创建主题：默认把选中主题作为父级，也可选择其他已存在父主题或直接创建根主题；创建后自动选中新主题。用户仍可在知识主题详情或图谱侧栏编辑名称、别名、笔记及关系，增加子主题、设置或更换父主题、删除单条 hierarchy/related 关系，并查看影响范围。设置父主题前必须做 DAG 成环校验；`related` 的两端可互换，不能被当作父子关系。
...
维护由用户手动触发，任务扫描整个 active Concept 图谱；当前 Concept、当前 Session、选中的 Concept 集合或选中的 KnowledgeUnit 集合只作为附加关注提示，不会裁剪图谱输入。
...
- 力向布局对 hierarchy 关系增加吸引约束，使子 Concept 倾向于靠近父 Concept；一个子 Concept 有多个父节点时，布局在多个父节点之间取折中位置。D3 模拟按结构分量施加独立重力中心，展开时只加热受影响分量，避免不相连的图团整体平移；弱 association/co-occurrence 边仍可显示，但不参与分量拖拽隔离；
- Ctrl/Cmd 单击和框选用于多选；
